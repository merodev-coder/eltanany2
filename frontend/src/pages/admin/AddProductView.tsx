...
const specFields = [
  { key: 'المعالج', label: 'المعالج (CPU)' },
  { key: 'كرت الشاشة', label: 'كرت الشاشة (GPU)' },
  { key: 'الرام', label: 'الرام (RAM)' },
  { key: 'التخزين', label: 'التخزين (Storage)' },
];
...
export default function AddProductView() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!name.trim() || !brand.trim() || !price || !stock || !description.trim()) {
    ...
    try {
      ...
      setCategory('laptop');
      setDescription('');
      setSpecs({ المعالج: '', 'كرت الشاشة': '', الرام: '', التخزين: '' });
      setImages([]);
    } catch {
  ...
  return (
    <div className="bg-white shadow-sm rounded-card p-6 sm:p-8">
      <div className="mb-8">
        ...
        <p className="font-body text-sm text-slate mt-1">أضف منتجاً جديداً إلى المتجر مع جميع التفاصيل</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
          <div>
            ...
            <input type="text" value={name} onChange={e => setName(e.target.value)} className={inputClass} placeholder="مثال: ASUS ROG Strix G16" />
          </div>
          <div>
            <label className="block font-body text-sm font-medium text-[#18181B] mb-2">الماركة</label>
            <input type="text" value={brand} onChange={e => setBrand(e.target.value)} className={inputClass} placeholder="مثال: ASUS" />
          </div>
          <div>
          ...
          <div>
            <label className="block font-body text-sm font-medium text-[#18181B] mb-2">السعر قبل الخصم (اختياري)</label>
            <input type="number" min="0" value={oldPrice} onChange={e => setOldPrice(e.target.value)} className={inputClass} placeholder="0" />
          </div>
          <div>
            <label className="block font-body text-sm font-medium text-[#18181B] mb-2">الكمية في المخزن</label>
            <input type="number" min="0" value={stock} onChange={e => setStock(e.target.value)} className={inputClass} placeholder="0" />
          </div>
          <div>
        ...
        <div>
          <label className="block font-body text-sm font-medium text-[#18181B] mb-3">المواصفات التقنية</label>
          <div className="border border-steel-light rounded-xl overflow-hidden">
            <table className="w-full">
              <thead>
                <tr className="bg-steel-light/50">
                  <th className="text-right py-3 px-4 font-body text-xs text-slate font-medium w-1/3">المواصفة</th>
                  <th className="text-right py-3 px-4 font-body text-xs text-slate font-medium">القيمة</th>
                </tr>
              </thead>
              <tbody>
                {specFields.map(field => (
                  <tr key={field.key} className="border-t border-steel-light">
                    <td className="py-3 px-4 font-body text-sm text-[#18181B]">{field.label}</td>
                    <td className="py-2 px-4">
                      <input
                        type="text"
                        value={specs[field.key] || ''}
                        onChange={e => setSpecs(prev => ({ ...prev, [field.key]: e.target.value }))}
                        className="w-full h-9 px-3 rounded-lg bg-steel-light border-0 font-body text-sm text-[#18181B] outline-none focus:ring-2 focus:ring-ignition-start/30"
